...
const Payments: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPayments = async () => {
      try {
        const response = await axios.get('http://localhost:5002/api/payments?limit=100');
  ...
  const handlePaymentToggle = async (paymentId: string, isPaid: boolean) => {
    try {
      // Find the payment to get loan ID
      ...
      if (isPaid) {
        try {
          // First, get the loan details to check loan term and existing payments
          const loanResponse = await axios.get(`http://localhost:5002/api/loans/${payment.loanId}`, {
            headers: {
          ...
          // Calculate the next payment month
          let nextMonth = 1;
          if (loan.payments && loan.payments.length > 0) {
            // Find the highest month number and add 1
            const maxMonth = Math.max(...loan.payments.map((p: any) => p.month));
            nextMonth = maxMonth + 1;
          }

          // Check if we're within the loan term
          if (nextMonth > loan.loanTerm) {
            toast({
              title: 'Payment Error',
              description: `Cannot record payment for month ${nextMonth}. Loan term is ${loan.loanTerm} months.`,
              variant: 'destructive'
            });
            return;
          }
          ...
          const response = await axios.post(`http://localhost:5002/api/loans/${payment.loanId}/payments`, {
            month: nextMonth,
            amount: payment.monthlyEMI,
            paymentMethod: 'manual',
            notes: `Payment for month ${nextMonth} marked as paid via dashboard`,
            receivedBy: user._id
          }, {
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`,
          ...
          if (response.status === 200) {
            toast({
              ...
              description: `Payment for month ${nextMonth} has been recorded successfully.`,
            });
            // Refresh payments data
            const paymentsResponse = await axios.get('http://localhost:5002/api/payments?limit=100');
        ...
        // For now, just show a message since removing payments isn't implemented
        toast({
                ...
                <tr key={payment._id} className="border-b border-border hover:bg-muted/20 transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">{payment.applicantEmail || 'No email provided'}</div>
                    </div>
                  </td>
